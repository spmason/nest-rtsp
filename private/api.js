...
function deviceAccessRequest( method, call, localpath, payload = '' ) {
	...
	if ( 'POST' === method && payload && '' !== payload ) {
...
function deviceAccessResponse( method, call, response ) {
	...
	switch( call ) {
		...
		case 'stopStream':
			...
			break
		case 'fanMode':
			if( 'Activate Fan' === document.getElementById( 'btnFanMode' ).textContent )
...
function onTemperatureSetpoint() {
	...
	if( '°F' === document.getElementById( 'heatUnit' ).innerText ) {
		heatCelsius = ( heatCelsius - 32 ) * 5 / 9
	}
	if( '°F' === document.getElementById( 'coolUnit' ).innerText ) {
